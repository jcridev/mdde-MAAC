...
(cd ${COMPOSE_DIR}/scripts/ && sh maac_retrieve_results.sh ${PFX}maac_dn_sm0_b1_10mrb ${WORK_DIR}/res_maac/maac_dn_sm0_b1_10mrb)
...
(cd ${COMPOSE_DIR}/scripts/ && sh maac_retrieve_results.sh ${PFX}maac_wdn_sm0_b1_10mrb ${WORK_DIR}/res_maac/maac_wdn_sm0_b1_10mrb)

# With do-nothing, consider storage, 80 fragments, bench at every step
mkdir -p ${WORK_DIR}/res_maac/maac_dn_b1_f80
(cd ${COMPOSE_DIR}/scripts/ && sh maac_retrieve_results.sh ${PFX}maac_dn_b1_f80 ${WORK_DIR}/res_maac/maac_dn_b1_f80)

# Without do-nothing, consider storage, 80 fragments, bench at every step
mkdir -p ${WORK_DIR}/res_maac/maac_wdn_b1_f80
(cd ${COMPOSE_DIR}/scripts/ && sh maac_retrieve_results.sh ${PFX}maac_wdn_b1_f80 ${WORK_DIR}/res_maac/maac_wdn_b1_f80)
